 ...
 *
 * @function
 * @param {Object} initial_state - initial redux store state
 * @param {Object.<string, reducer>} action - object of `{[action type]: (state, action) => new_state }`
 * @return {reducer} - reducer function.
 */
export const mk_reducer = (initial_state, actions) => (state=initial_state, action) => {
 ...
 */
export const simple_selectors = (keys) => {
  let sel_keys = Array.isArray(keys) ? keys : Object.keys(keys);
  return sel_keys.reduce((obj, key) => ({
